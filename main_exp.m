...
data.tStart  =   time;
data.responses = [];
data.trial_times = zeros(data.num_trials,11);
for tr=1:data.num_trials
    ...
    if data.MEG, cgrect(-500,380,2*data.pxl.fix,2*data.pxl.fix,[1 1 1]); end % optional to have a white spot in corner to record precise screen timings with photodiode
    cgflip(bg_col(1),bg_col(2),bg_col(3));    
    ...
    % record response
    respStart = time; %within trial timer
    ...
    data.trial_times(tr,1)=tStart1;
    data.trial_times(tr,2)=tStart2;
    data.trial_times(tr,3)=instructStart;
    data.trial_times(tr,4)=stimStart;
    data.trial_times(tr,5)=stimEnd;
    data.trial_times(tr,6)=fixStart;
    data.trial_times(tr,7)=delayEnd;
    data.trial_times(tr,8)=respStart;
    data.trial_times(tr,9)=respEnd;
    data.trial_times(tr,10)=itiStart;
    data.trial_times(tr,11)=itiEnd;
